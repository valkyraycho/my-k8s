//! A deduplicating work queue, modeled on client-go's `workqueue.Type`.
//!
//! Controllers never process watch events directly — they enqueue a KEY (here,
//! a resource name) and a worker drains keys and reconciles. The queue's job:
//!   (a) DEDUP — a burst of events for one object collapses to ONE reconcile;
//!   (b) SAFE REQUEUE — a key re-added WHILE it's being processed gets
//!       reprocessed afterward (never lost, never two workers on one key).
